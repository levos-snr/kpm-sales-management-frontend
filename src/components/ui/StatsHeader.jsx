...
import { MoreHorizontal, ArrowUpRight, ArrowDownRight, Download, Bell, Eye } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 p-4 bg-white rounded-lg">
      {stats.map((item, index) => (
        ...
        >
          ...
          >
            <DropdownMenu onOpenChange={(open) => open ? handleDropdownOpen(index) : handleDropdownClose()}>
            ...
            <CardContent className="p-4 pt-8 flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {item.badge && (
                    <span className={`px-2 py-0.5 text-xs bg-${item.badge.color}-100 text-${item.badge.color}-700 rounded-full font-medium`}>
                      {item.badge.text}
                    </span>
                  )}
                </div>
